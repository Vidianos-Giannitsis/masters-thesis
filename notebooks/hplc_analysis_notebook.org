...
** Πείραμα 06-06
Αυτό είναι το τελευταίο πείραμα HPLC που έγινε το οποίο είχε τα υδρολύματα της όρκας και το υδρόλυμα που χρησιμοποιήθηκε για την χώνευση. Επίσης, είχε και ένα δείγμα της διαλυτής φάσης του ανεπεξέργαστου FW για να έχουμε μία εικόνα του τι έχει. Αρχικά θα μετατραπούν τα αποτελέσματα του σε συγκεντρώσεις και μετά θα χωριστούν ανά πείραμα.

*area_to_conc_06_06*
#+NAME: area_to_conc_06_06
#+BEGIN_SRC julia

  date = "06_06"
  df0606_conc = process_area_data2(get_area_csv(date, "1"), get_conc_csv(date, "1"))

  fw_df = df0606_conc[1, 2:end]
  fw_conc = Vector(fw_df)
  fw_diluted = fw_conc./3
...
Εκτός από τις συγκεντρώσεις του FW, υπολογίζεται και η συγκέντρωση αυτού δια 3 για να παρατηρηθεί αν είναι αντίστοιχο των ζυμώσεων την στιγμή 0. Είναι κοντά και σε κάποιες περιπτώσεις λίγο χαμηλότερο από αυτά. Αυτό είναι αναμενόμενο επειδή μόνο και μόνο η αραίωση αυξάνει σε ένα βαθμό το διαλυτό COD του υποστρώματος, άρα αναμένεται πως κάποια σάκχαρα διαλυτοποιήθηκαν κατά την αραίωση.
